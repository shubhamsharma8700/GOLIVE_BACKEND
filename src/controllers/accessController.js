...
export default class AccessController {
  static async findViewerByEventAndEmail(eventId, email) {
    if (!eventId || !email) return null;
    const resp = await ddbDocClient.send(
      new ScanCommand({
        TableName: VIEWERS_TABLE,
        FilterExpression: "eventId = :eventId AND email = :email",
    ...
    try {
      ...
      // PASSWORD ACCESS
        ...
        if (flowMode === "register") {
          ...
          const existingEmailViewer = await AccessController.findViewerByEventAndEmail(
            eventId,
            primaryEmail
          );

          if (existingEmailViewer) {
            return res.status(409).json({
              message:
                "Email already registered. Please login with the password sent previously.",
              viewerId: existingEmailViewer.viewerId,
            });
          }
  ...
  static async resendPassword(req, res) {
    try {
      const { eventId } = req.params;
      if (!eventId) return res.status(400).json({ message: "Missing eventId" });

      const { email, clientViewerId, firstName, lastName } = req.body || {};
      ...
      const event = eventResult.Item;
      const storedPassword = event.accessPassword || event.password || null;
      if (!storedPassword)
        return res.status(500).json({ message: "Event password not configured" });

      const viewer = await AccessController.findViewerByEventAndEmail(eventId, email);
      if (!viewer)
        return res.status(404).json({ message: "Viewer not registered yet" });

      const resolvedFirstName = firstName || viewer.formData?.firstName || viewer.name?.split(" ")?.[0] || "Viewer";
      const resolvedLastName = lastName || viewer.formData?.lastName || viewer.name?.split(" ").slice(1).join(" ") || "";

      await sendPasswordFromServer({
        eventId,
        clientViewerId: clientViewerId || viewer.clientViewerId,
        email,
      ...
      return res.status(200).json({
        success: true,
        message: "Password email resent successfully",
        viewerId: viewer.viewerId,
      });
    } catch (error) {
      console.error("resendPassword error", error);
      return res.status(500).json({ message: "Unable to resend password" });
    }
  }
